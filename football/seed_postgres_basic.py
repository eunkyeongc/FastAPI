...
# 2026. 7. 9.
#  
# fastapi\football\seed_postgres_basic.py
#
...
# 따라서, 테스트를 실행하기 전에 이 파일을 한 번 실행해서 CSV 데이터를 PostgreSQL 데이블에 넣어두어야 한다.
#
# 실행할때마다 기존 테이블을 drop_all()로 삭제하고 다시 만든다.
# DB에 중요한 데이터가 있다면 절대 실행하면 안된다.
# ============================================================================

from __future__ import annotations # 함수나 클래스의 타입 표시를 바로 평가하지 않고 문자열처럼 저장하도록 만드는 옵션

import csv
from datetime import date
from pathlib import Path # 파일의 경로를 찾기 쉽게해주는 함수
...
BASE_DIR = Path(__file__).resolve().parent  #football 폴더를 찾겠다는 것.

# CSV 원본 데이터가 들어있는 폴더
DATA_DIR = BASE_DIR / "data"  # football/data  폴더

def parse_date(value: str) -> date:
    """
    CSV에서 읽은 날짜 문자열을  Python date 타입의 객체로 변환한다. 
...
def read_rows(filename: str) -> list[dict[str, str]]:
    ...
    row["player_id"]처럼 컬럼명으로 값을 꺼낼수 있다.
    """
...
def seed() -> None:
    ...
    with Session(engine) as session:
        # 1. player 테이블 적재 - player는 performance와 tea_player에서 참조하므로 먼저 넣는다.
        players = [
            models.Player(
        ...
        # 2. league 테이블 적재 - League는 team에서 참조하므로 team보다 먼저 넣는다.
        leagues = [
            models.League(
        ...
        # player와 league 행이 같은 트랜젹션 안에서 실제 테이블에 먼저 보이게 된다.
        ...
        # 4. team_player 연결 테이블 적재 - Team과  Player의 다대다 관계를 표현하는 중간 테이블
        team_players = [
            models.TeamPlayer(
                team_id = int(row["team_id"]),
                player_id=int(row["player_id"]),
        ...
        # 5. performance 테이블 적재 - Performance는 player_id로 Player를 참조한다.
        performances = [
            models.Performance(
        ...
        # SQLAlchemy 세션에 "저장 예정" 상태로 올라간 것읻.
        # commit()을 호출해야 PostgreSQL테이블에 INSERT가 반영된다.
        session.commit()
