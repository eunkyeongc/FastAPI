...
# Streamlit도 따로 streamlit run으로 실행을 해야한다.
# =====================================================================
...
st.set_page_config(page_title='나의 할 일 관리',
                   page_icon='',
                   layout='centered')
...
</style> ''', unsafe_allow_html=True) # unsafe_allow_html=True --> html/css가 먼저 적용이 되도록 함.
...
def get_headers() -> dict:
    """ 로그인 후 API 요청에 매번 붙여야 하는 인증 헤더를 만들어 반환한다. """
    return {'Authorization': f'Bearer {st.session_state.access_token}'}

def logout():
    """ 세션 상태를 비워서 로그아웃 처리한다.(서버에 요청을 보낼 필요가 없다. -> JWT는 stateless 방식이기 때문)"""
    st.session_state.access_token = None
    st.session_state.user_email = None

def extract_error_message(res: requests.Response, fallback: str) -> str:
    """ 
    FastAPI 에러 응답에서 사람이 읽을 수 있는 메시지만 뽑아 낸다.
    ...
    try:
        ...
        return f'서버 오류(status {res.status_code}). unvicorn 터미널을 확인해주세요.'

    if isinstance(detail, list):
        messages = []
        for err in detail:
            # err이 dict라면(딕셔너리라면) msg키의 값을 msg변수에 넣어라, 아니면 err을 문자열로 형변환해서 msg변수에 넣어라.
            msg = err.get('msg', '') if isinstance(err, dict) else str(err)
            # 'Value error, '를 없애겠다. 대신, 단어를 치환한다. 바꾼다. -> replace())
            msg = msg.replace('Value error, ', '')
            messages.append(msg)

        #messages가 있으면 메시지 사이사이에 엔터를 삽입, 아니면 그냥 fallback 변수에 있는 값이 반환
...
if res.status_code == 401:
    # 토큰이 만료되었었거나, 유효하지 않으면 자동으로 로그아웃 처리
...
done_count = sum(1 for t in todos if t['is_done']) # 완료된 일 체크
...
for todo in todos:
    ...
    with title_col:
        st.markdown(f'<div class="todo-card {card_class}">{todo["title"]}</div>', unsafe_allow_html=True)
