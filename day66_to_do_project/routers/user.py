...
# BackgroundTasks : 응답을 먼저 클라이언트에게 보내고, 그 이후에 처리해도 되는 작업을 뒤에서 실행하게..
...
def send_welcome_email(email: str):
    ...
    실제 메일 발송 대신 5초 걸리는 작업으로 대신 <- BackgroundTasks로 등록하면 클라이언트는 이 5초를 안 기다리고 바로 응답을 받는다.
    """
    import time
    time.sleep(5)
...
def signup_user_handler(body: UserSignUpRequest, 
    ...
    user = service.signup(body)  # 이메일 중복 체크, 비밀번호 해싱 등 처리, 라우터는 그 결과(user)를 받아서 백그라운드 태스크만 등록하고 반환
...
def refresh_access_token_handler(body: RefreshTokenRequest,
                                 service: UserService = Depends(get_user_service)):
    return service.refresh(body.refresh_token)

@router.post('/users/logout', status_code=status.HTTP_200_OK)
def logout_user_handler(body: RefreshTokenRequest, 
                        service: UserService = Depends(get_user_service)):
