...
# status : HTTP 상태코드를 숫자 대신 읽기 쉬운 이름(상수)으로 사용하게 된다.
...
# 3. get_todos_hander() 함수 본문이 실행된다.
@router.get('/todos', response_model=list[TodoResponse], status_code=status.HTTP_200_OK)
def get_todos_handler(user_id: int = Depends(get_current_user_id), 
...
def delete_todo_hander(todo_id: int,
                       user_id: int = Depends(get_current_user_id),
                       service: TodoService = Depends(get_todo_service)):
