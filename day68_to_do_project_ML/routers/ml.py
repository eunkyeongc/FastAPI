...
# 2026. 8. 19.
# to_do_ project
# day68_to_do_project_ML\routers\ml.py

# 카테고리 예측 모델의 '정확도'를 확인할 수 있는 모니터링 전용 엔드포인트
# MLOps핵샘 개념 --> 모델을 배포하고 끝!이 아니라, 배포 후에도 계속 지켜본다(모니터링)는 것이 이 파일의 존재 이유이다.
# 실무에서는 정확도가 서서히 떨어지는 현상 (mdel drift)을 감지하기 위해 이런 모니터링 지표를 반드시 별로로 관리한다.
# ========================================================================================================================
...
def get_model_accuracy_handler(session=Depends(get_session)):
    """
    final_category가 채워진(=사용자가 확인/수정을 완료한) Todo만 '정답이 있는' 데이터로 취급해서 정확도 계산 대상으로 삼는다.
    ...
    # predicted_category와 final_category가 정확히 일치하는 갯수를 알고싶다.
    # models.py에서 두 개의 컬럼을 나누었다.
    correct_stmt =select(func.count()).select_from(Todo).where(
        Todo.final_category.is_not(None),  # SQL문으로 --> WHERE final_category IS NOT NUL
