...
# 2026. 08. 13.
# to_do_ project
# day66_to_do_project\auth\jwt.py
 
# JWT 발금/검증 모듈
# Access Token
# Refresh Token

# 로그인 시 --> access_token + refresh_token 둘 다 발급, refresh_token은 DB에 저장.
# 30분(ACCESS_TOKEN_EXPIRE_MINUTES) 후 --> access_token은 만료
# 할 일 등록(POST), 할 일 완료 체크(PATCH), 할 일 확인(GET) 하기 위해 다시 로그인 시  --> access_token 재발급
# 일 처리  완료 후 로그 아웃 하면 access_token 만료.
# 7일(REFRESH_TOKEN_EXPIRE_DAYS) 후 refresh_token도 만료
# 이후 로그인 시 --> access_token + refresh_token 둘 다 재발급
...
import jwt  # 라이브러리는 pyjwt(uv add pyjwt or pip install pyjwt)
...
ACCESS_TOKEN_EXPIRE_MINUTES = 30    # 자주 쓰이는 토큰이니 짧게 30분 동안 로그인 상태 유지
...
def _create_token(user_id: int, token_type: str, expires_delta: timedelta) -> str:
    ...
    이름 앞 _(언더스코어)의 역할 --> 이 파일 밖에서는 직접 부르지 말고, 아래 두 함수를 통해서만 호출해라 라는 파이썬의 관례
    """
    payload = {
        'user_id': user_id,
        'type': token_type,     # 'access'/'refresh' 두가지 type이 있음.
...
def decode_token(token: str, expected_type: str) -> int:
    ...
    except jwt.ExpiredSignatureError:   # 토근이 만료된 경우
