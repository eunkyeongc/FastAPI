...
# 2026. 08. 13.- > 8. 19. 수정
# to_do_ project
# day66_to_do_project\services\todo_service.py
#
# Todo 관련 "업무 규칙"을 담당하는 계층
# DB 쿼리 자체는 직접하지 않고, TodoRepository에게 다 위임한다.
# 없으면 "404 에러를 낸다." 수정할 때 "title/is_done"만 부분 반영한다" 등의 판단들을 정의
...
class TodoService:
    # category_service: CategoryPredictionService |None=None 이유는?
    # ML 모델이 아직 로드되지 않은 상태에서도  TodoService자체는 정상 작동해야 하기 때문이다.
    ...
    def get_todo(self, todo_id: int, user_id: int) -> Todo:
        ...
        # Repository에서 None을 반환 -> 404 에러(에러를 보낼지 통과를 시킬지를 이 계층에서 판단하여 처리한다.)
    ...
    def update_category(self, todo_id:int, category:str, user_id:int) -> Todo:
        ...
        todo.final_category = category   # 확인/수정 되는 지점
