...
# 2026. 08. 12.
# to_do_ project
# day66_to_do_project\database\db_connection.py
#
...
# 엔진 생성 - DB와 실제 연결을 관리하는 객체
# echo=True --> 실행되는 SQL을 터미널에 출력(학습용, 실무에서는 echo=False 사용)
...
SessionFactory = sessionmaker(
    ...
    expire_on_commit=False, # commit이후에도 객체 값을 메모리에서 계속 사용 가능하다.
...
def get_session():
    ...
    # 함수 안에 yield가 있으면, yield 시점까지 실행한 뒤, 그 값(session)을 라우터 함수(실제 업무 수행 함수)의 매개변수로 전달한다.
    #   --> 라우터 함수 처리가 끝나면, FastAPI가 yield 다음 줄(finally)을 실행해서 세션을 자동을 닫아준다.
