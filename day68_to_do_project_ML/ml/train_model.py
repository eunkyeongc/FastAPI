# ===============================================================================================================================
# 2026. 8. 18.
# day68_to_do_project_ML\ml\train_model.py
#
# - Todo 제목 텍스트로 카테고리(업무/개인/긴급)를 분류하는 모델을 학습니다.
...
# __file__.resolve().parent : 이 스크립트가 어느 폴더에 있든 항상 스크립트 자신의 위치를 기본으로 경로를 계산한다. (실행 위치에 흔들리지 않는다.)
BASE_DIR = Path(__file__).resolve().parent
# print(BASE_DIR)  # C:\Users\Administrator\bigdata2026\fastapi\day68_to_do_project_ML\ml
...
def build_pipeline() -> Pipeline:
    ...
    --> 왜 묶어?
        ...
        두 개체를 각각 파일로 저장/로드해야한다.
        파이프라인으로 묶으면 한 줄로 벡터화+분류가 한 번에 처리되고, 저장/로드도 파일 하나로 끝난다.

    TfidVectorizer(ngram_range(1,2)): 단어 하나(unigram) 뿐 아니라 연속된 두 단어 (bigram)까지 특징을 사용한다.
                                      예) "회의 자료" --> "회의", "자료", "회의 자료", 짧은 한국어 제목에서 문맥 정보를 조금 더 살릴 수 있다.
    """
...
def main():
    ...
    # 입력(피처, 독립변수) --> df['title']  --> X(대문자)
    # 결과(타겟, 종속변수) --> df['category'] --> y(소문자)
    ...
    # 학습에 사용되지 않은  X_test로 예축, 진짜 정답(y_test)과 비교
    ...
    #                 pickle과 비슷하지만, numpy 배열이 많은 sklearn객체 저장에 더 최적화 되어 있다.
    joblib.dump(pipeline, model_path)

    # 모델 파일만 저장하면 "언제, 얼마나 정확했는지" 기록이 안 남는다.
    # 최소한의 버전 관리로 메타데이터를  JSON으로 같이 남겨둔다.
    # isoformat() --> ISO 8601 날짜 시간으로 나태내는 국제표준형식 YYY-MM-DD.....
    ...
    # 버전이 올라갈때마다 main.py 코드를 수정할 필요없이, 이 파일 하나만 최신모델로 덮어쓰기하면 서버가 자동으로 최신 버전을 로드하게 된다.
