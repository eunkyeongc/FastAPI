...
# 2026. 08. 19.
# to_do_ project
# day68_to_do_project_ML\ml\retrain.py
#
# 사용자가 실제 수정한 카테고리(final_category)를 새 학습 데이터로 삼아 모델을 재학습한다.
# 이 스크립트도 FastAPI 서버와 분리된 별도 프로세스로 실행한다.
#   (수동 실행 -> 이후 필요하면 cron/스캐줄러로 자동화)
# 시간이 지날수록 실제로 만든 Todo데이터가 쌓이면서 모델이 점점더 우리 말투/패턴에 맞게 똑똑해지는 과정을 볼 수 있다.
# ==============================================================================================================
...
def load_user_corrected_data() -> pd.DataFrame:
    """
    사용자가 final_category를 직접 지정한(확인/수정) Todo만 새 학습 데이터로 사용한다.
    FastAPI와 별개로 요청 흐름 밖에서 실행되므로 SessionFactory()를 직접 호출해서 세션을 만들고, 끝나면 직접 close()한다.
    """

    session = SessionFactory()
    try:
        # final_category가 NULL이 아닌(사용자가 한 번이라도 확인한)행만 조회
        stmt =select(Todo.title, Todo.final_category).where(Todo.final_category.is_not(None))
        rows = session.execute(stmt).all()
    finally:
        session.close()

    if not rows:
        # 아직 아무도 카테고리를 확인/수정하지 않은 경우 빈 DataFrame을 반환
...
def main():
    ...
    X_train, X_test, y_train, y_test = train_test_split(combined_df['title'], combined_df['category'], test_size=0.2, random_state=42)
    ...
    metadata = {'version': version,
                ...
                'source': "original + user_corredcted"}
