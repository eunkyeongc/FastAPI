"""
langchin_practice/00_check_setup.py
-------------------------------------
설치 확인 + Gemini API key 연결 확인 테스트

이 파일의 목적은
1. .env에 넣어둔  GOOGLE_API_KEY가 제대로 읽히는지 확인
2. GEMINI모델에 실제로 요청을 보내고, 응답을 받아오는지 확인
3. 여기서 에러가 나면 이후 모든 실습이 진행되지 않으므로 꼭 통과시킬것!
"""
...
# Gemini 모델을 사용할 수 있게 감싸주는 객체를 생성 - 무료 티어가 있고, 이미지(비전)입력도 지원하는 가벼운 모델 설정 --> 2.5 flash
