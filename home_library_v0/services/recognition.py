...
def normalize_isbn(value: str) -> str | None:
    ...
    ISBN-10자리 / ISBN-13자리 두 종류가 있다.
    """
    digits = re.sub(r"[^0-9Xx]", "", value) # 예) 978-89-1234-567-8 --> 9788912345678 추출

    if len(digits) == 10:   # ISBN-10자리 검증
    ...
    return None  # "ISBN-10자리 / ISBN-13자리"이 아니면  None을 반환
...
def extract_isbn(image_path) -> str | None:
	...
	# ── v3 대비 추가된 부분 ──
    # :=(바다코끼리 연산자, walrus operator) -> 대입과 조건 확인를 한줄에 동시에 처리
    #   --> normalize_isbn(candidate)호출한 결과를 isbn에 담고, 조건이 참이면 isnb을 리턴한다.               
    #
