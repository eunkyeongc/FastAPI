...
# OCR 기능을 삽입하는 기능을 넣는 파일 --> main.py에 넣지않고 별도 파일로 빼서 관심사를 분리
# main.py는 API요청을 어떻게 처리할지에만 집중
# recongnition.py는 OCR을 어떻게 돌릴지에 집중. 나중에  OCR방식을 바꾸더라도(다른 OCR 엔진으로 교체) 이 파일만 수정하면 된다.

 # uv add pytesseract --> OCR 인식해주는 라이브러리
# =============================================================================================================================
...
def extract_isbn(image_path) -> str | None:
    ...
    매개변수 --> image_path: 이미지의 경로 
    반환값 --> isbn 문자열 또는 못 찾으면 None이 반환
    """
    try:
    ...
    with Image.open(image_path) as source:
        ...
        # image = ImageOps.grayscale(source) --> 컬러 사진을 흑백(그레이스케일)으로 변환
        ...
        # config='--psm 11' --> PSM(페이지 분석 모드) 11번 : 정해진 문단 구조없이, 흩어진 텍스트를 모두 모아라.
        # pytesseract.image_to_string(...) --> 이미지를 Tesseract 엔진에 넘겨서 이안에 있는 글자를 다 텍스트로 뽑아줘.
