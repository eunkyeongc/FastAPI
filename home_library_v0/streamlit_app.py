...
# Streamlit이 이 주소로 HTTP요청을 보내고 데이터를 주고 받는다.
...
#     # files={'image':(파일명, 파일내용바이트, 파일타입)} --> FastAPI에 upload File 매개변수 image와 일치해야한다.
#     r = requests.post(f'{API}/books/scan', files={'image': (image.name, image.getvalue(), image.type)})
...
for book in requests.get(f'{API}/books').json():
    st.write(f'{book["title"]} ({book["recognition_status"]})') # '책 제목 (상태)'형태로 결과를 보여 줌.
