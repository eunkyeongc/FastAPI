...
# 예광탄(tracer bullet) 방식을 활용한 아주 얇은 코드
...
from services.recognition import extract_isbn   # ver.3에서 추가

UPLOAD_DIR = Path('uploads')    # 현재 프로그램 실행 위치를 기준으로 uploads라는 폴더 경로를 나타내는 객체 만들어라.
UPLOAD_DIR.mkdir(exist_ok=True)   # 폴더가 이미 존재해도 오류로 처리하지말고 uploads 디렉토리 만들어라.

# models.py에서 정의한 Book, RedingStatus, Review 클래스들을 실제 PostgreSQL 테이블로 생성하는 역할
...
# ---------- POST /books/scan --> 사진을 업로드해서 책 한 권을 등록하는 API : ver.2 (진짜 이미지인지 검증) -----------------

@app.post('/books/scan', status_code=status.HTTP_201_CREATED)
def scan_book(image: UploadFile = File(...), db: Session = Depends(get_db)):
    # Ver.2에서는 raw 변수에 이미지를 받아만 둔다.
    # 저장은 검증을 통과한 다음에 한다.
    raw = image.file.read()

    try:
        # io.BytesIO(raw) --> 방금 읽은 바이트 데이터를 메모리 안의 파일처럼 다루게 해주는 도구
        #                     (드스크에 진짜 파일을 만들지 않고도 파일인것처럼 열어볼 수 있다.)
        # Image.open(...) --> PIL(pillow) 라이브러리로 이 바이크가 이미지로 열리는지 시도
        # with ... as probe:  --> 확인이 끝나면 자동을 메모리를 정리해준다.(파일을 닫는다.)
        with Image.open(io.BytesIO(raw)) as probe:
            #probe.verify() --> 이 파일이 진짜로 손상되지 않은 이미지 형식이 맞는지 내부적으로 검사(PIL=pillow)
            probe.verify()

    # UnidentifiedImageError --> PIL이 아예 이미지 형식으로조차 인식하지 못했을때 발생하는 에러
    # OSError --> 파일이 중간에 잘렸거나, 손상된 경우 등 좀 더 넓은 범위의 파일 관련 에러
    except (UnidentifiedImageError, OSError):
        #  HTTP 상태코드 415(Unsupported Media Type, 지원하지 않는 파일 형식) 함수 실행 중단시킨다.
        # raise를 만나는 순간 저장, DB 등록은 실행되지 않는다.
    ...
    #ver.3 --> extract_isbn(path) : 방금 저장한 사진 파일 경로를 넘겨서 ISBN처럼 생긴 문자열을 뽑아논다.
...
def list_books(db:Session=Depends(get_db)):
    # select(Book) : Books 테이블에서 모두 가져와라. SQL SELECT 문을 파이썬 코드로 작성한 것.
